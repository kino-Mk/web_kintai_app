...
document.getElementById('btn-export-monthly-csv').addEventListener('click', async () => {
    ...
    try {
        ...
        if (snapshot.empty) {
            alert(`${selectedMonth}月次 (${prevYear}/${prevMonth}/21 〜 ${year}/${month}/20) の打刻データはありません。`);
            return;
        }
...
async function completeApplication(docId) {
    if (!confirm("この申請を承認し「処理済み」にしますか？\n（承認録として打刻データが自動生成されます）")) return;
    ...
    try {
        ...
        // 成功メッセージ（リアルタイムリスナーが動くのでアラートは控えめ）
        console.log("Application completed and attendance record created.");
...
document.getElementById('btn-delete-selected-att').addEventListener('click', async () => {
    ...
    if (checkboxes.length === 0) {
        alert("削除する打刻データを選択してください。");
        return;
    }

    if (!confirm(`選択した ${checkboxes.length} 件の打刻データを本当に削除しますか？\n（この操作は元に戻せません）`)) {
        return;
    }
...
document.getElementById('btn-calendar-load').addEventListener('click', () => {
...
// 休日自動設定ボタンのイベント
document.getElementById('btn-calendar-auto-set').addEventListener('click', () => {
    const month = document.getElementById('calendar-month-selector').value;
    if (month) {
...
async function autoSetHolidays(monthStr) {
    if (!confirm(`${monthStr} の土日と日本の祝日を自動的に休日設定しますか？`)) return;
    ...
    try {
        ...
        if (count > 0) {
            await batch.commit();
            alert(`${count} 日分を休日として設定しました。`);
            renderCalendar(monthStr);
        } else {
            alert("設定対象の日が見つかりませんでした。");
        }
...
document.getElementById('btn-detail-delete').addEventListener('click', () => {
    ...
    db.collection("employees").doc(currentDetailEmpId).delete()
        .then(() => {
            alert("削除しました。");
...
document.getElementById('btn-detail-att-load').addEventListener('click', () => {
    if (!currentDetailEmpId) return;
...
document.getElementById('btn-detail-att-delete').addEventListener('click', async () => {
    ...
    if (!confirm(`選択した ${checkboxes.length} 件の打刻データを削除しますか？`)) return;
...
document.getElementById('btn-add-att-manual').addEventListener('click', () => {
    if (!currentDetailEmpId) return;
...
function deleteCompletedApplicationForDetail(docId, empId, filterType, filterValue) {
...
// 処理済み申請のフィルタボタン
document.getElementById('btn-detail-app-filter').addEventListener('click', () => {
    if (!currentDetailEmpId) return;
    ...
    if (filterValue) {
        ...
        alert(filterType === 'month' ? "月を選択してください。" : "年を入力してください。");
    }
});
...
document.getElementById('btn-grant-leave').addEventListener('click', async () => {
    ...
    if (!date || isNaN(amount)) {
        alert("日付と日数を入力してください。");
        return;
    }

    if (!confirm(`${amount} 日の有給を付与しますか？`)) return;
...
async function relockAdmin() {
    try {
        ...
        alert("再ロックに失敗しました: " + error.message);
    }
}
